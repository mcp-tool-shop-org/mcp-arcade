...
2. Env oracle moves to `docker diff` for docker targets: `env_before`/`env_after` become the container filesystem delta, and the fixture's leak path is inside the container (`/tmp/leak.txt`). The directory sandbox stays for fixture and plain stdio targets.
